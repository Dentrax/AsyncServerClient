...
namespace ServerFramework
{
    public class Packet
    {
        ...
        public String ReadAscii(string encoding)
        {
            lock (m_lock)
            {
                if (!m_locked)
        ...
        public void WriteAscii(String value, string encoding)
        {
            lock (m_lock)
            {
                if (m_locked)
                ...
                byte[] codepage_bytes = Encoding.GetEncoding(encoding).GetBytes(value);
                //string utf7_value = Encoding.UTF7.GetString(codepage_bytes);
                //byte[] bytes = Encoding.Default.GetBytes(utf7_value);
